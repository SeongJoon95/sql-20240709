...
SELECT * FROM nures WHERE id = 'qwer1234';

-- SELECT * FROM nures WHERE id =:id AND password = :password;
SELECT * FROM nures WHERE id = 'qwer1234';

-- 회원가입
-- 최종 사용자 (이름, 아이디, 비밀번호, 비밀번호확인, 전화번호, 인증번호)
-- 요양사 테이블에 레코드(아이디, 암호화된 비밀전호, 전화번호, 가입경로, 이름)를 생성
-- 필요한 추가 데이터 : 가입경로 (화면에서 가져옴), 암호화된 비밀번호 (서버가 생성)

-- 레코드 삽입시 아이디 중복 검증
SELECT * FROM nures WHERE id = 'qwer1234';
-- 레코드 삽입시 전화번호 중복 검증
SELECT * FROM nures WHERE tel_number = '01012345678';
-- 절차상에 인증된 전화번호만 삽입 가능
SELECT * FROM tel_auth WHERE tel_number = '01012345678' AND auth_number = '0123';

INSERT INTO nures VALUES ('qwer1234', 'qwerasdzxc', '홍길동', '01012345678', 'HOME');
...
SELECT * FROM tel_auth WHERE tel_auth='01012345678';
...
UPDATE tel_auth SET auth_nunmber = '0123' WHERE tel_number = '01012345678';
...
-- 전체 게시물 수 및 전체 페이지 수 불러오기 --
-- 최종 사용자 () => (전체 고객수 수, 전체 페이지 수)
...
DELETE FROM customes WHERE costomer_number = 1;
...
VALUES('이영희', '부산광역시', 'qwer1234', null,'590816','부산광역시 부산진구');

-- 담당자(요양자) 검색
-- 최종 사용자 (이름)
-- 요양사 테이블에서 이름을 기준으로 입력한 이름과 같은 레코드를 조회
-- 필요 추가 데이터 : x
SELECT id, name, tel_number FROM nures WHERE name = '홍길동';

-- 별도로 테이블 이름 바꾸기
ALTER TABLE nures RENAME TO nurse;

-- 고객 상세보기 
-- 최종 사용자 (고개 정보) => 고객번호, 고객사진, 고객이름, 생년월일, 담당자 이름, 주소
-- 고객 테이블에서 특정 고객을 조회하여 반환, 담당자 이름을 반환하기위해 영양사 테이블을 조인
-- 필요 추가 데이터 : 고객번호(화면에서)
SELECT C.customer_number, C.profile_image, C.name, C.birth, N.id, N.name"담당자", C.address
FROM customer C INNER JOIN nurse N
ON C.charger = N.id
...
UPDATE goods SET count = count - 2 WHERE goods_number = 1; -- 사용한 굿즈가 없다면 수행 x

-- 고객 수정
-- 최종 사용자 (고객 사진, 고객 이름, 생년월일, 담장자, 주소)
-- 고객 테이블에서 특정 고객에 대한 레코드(고객사진, 고객이름, 생년월일, 담당자, 주소)를 수정
