...
-- 게시물 리스트 
SELECT 
	receipt_number, status, title, writer_id, writer_date, view_count
FROM board
WHERE title LIKE '%셋%' -- LIKE : 포함되어 있는 것을 가져옴
ORDER BY receipt_number DESC
LIMIT 0, 10; -- LIMIT (:page_number - 1) * 10, 10;

-- 게시물 작성
INSERT INTO board (title, contents, writer_id)
VALUES ('열둘번째 게시물', '안녕하세요', 'qwer1234');

-- 게시물 상세보기
UPDATE board SET view_count =  view_count + 1
WHERE receip_number = 1;
-- +
...
-- 게시물 삭제
DELETE FROM board WHERE receipt_number = 2;
